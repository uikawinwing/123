...
- Quick Reply / QR button 指输入栏上方的脚本库按钮：使用 `appendInexistentScriptButtons`，按钮事件用 `eventOn(getButtonEvent(...), ...)`；不要默认堆很多 QR button。
- 用户说“魔术棒”“魔法棒”“magicwand”“wand”“扩展菜单入口”或近义表达时，默认把脚本入口加到酒馆魔术棒 / `extensionsMenuButton` / `#extensionsMenu` 菜单中；点开后再显示交互菜单，不要使用 QR button，具体写法看 `.cursor/rules/脚本.mdc` 的“扩展菜单 / 魔术棒入口”。
